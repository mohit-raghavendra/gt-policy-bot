...
def get_answer_with_evidence(model, query, tokenizer, context, question, max_new_tokens, temperature, 
  ...
  outputs = model.generate(**inputs, max_new_tokens=max_new_tokens, temperature=temperature, 
                           num_return_sequences=3, return_dict_in_generate=True, output_scores=True)
...
def get_answer_without_evidence(model, tokenizer, context, question):
  ...
  outputs = model.generate(**inputs, max_new_tokens=128, return_dict_in_generate=True, 
                           num_return_sequences=3, output_scores=True)
  input_length = 1 if model.config.is_encoder_decoder else inputs.input_ids.shape[1]
  generated_tokens = outputs.sequences[:, input_length:].cpu()
  transition_scores = model.compute_transition_scores(outputs.sequences, outputs.scores, normalize_logits=True)
  transition_scores = transition_scores.cpu()
...
def run_test(test, model, query, tokenizer, with_evidence=True):
  ...
  for index, question in questions.iterrows():
    ...
    if with_evidence:
      llm_output, llm_input = get_answer_with_evidence(model, query, tokenizer, context, actual_question,
                                         max_new_tokens, temperature, num_evidence)
    else:
      llm_output, llm_input = get_answer_without_evidence(model, tokenizer, context, actual_question)
    with open(output_file, "a") as output:
      output.write(f'\n\n\n Question {number} \n\n LLM Input:\n\n {llm_input} \n\n LLM Output:\n\n{llm_output}')
